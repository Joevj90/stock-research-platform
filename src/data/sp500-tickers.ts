/**
 * S&P 500 ticker universe for the Valuation Screener.
 *
 * This is a STATIC snapshot list, not a live feed from the index
 * provider -- the app's FMP plan's index-constituent entitlement wasn't
 * confirmed, so rather than silently fail or half-build a live-fetch
 * path that might 402 on this account, this ships a known, real list of
 * S&P 500 members instead. Real limitation, stated plainly: index
 * membership changes a handful of times a year (additions/removals on
 * reconstitution), so this list will drift slightly out of date over
 * time. It is not fabricated data about any individual company -- it is
 * simply a ticker symbol list used to decide which real tickers to scan;
 * every price and fair-value number computed for each ticker is still
 * fetched and calculated for real, exactly as everywhere else in this
 * app.
 *
 * To refresh this list in the future: replace the array below with an
 * updated S&P 500 constituent list (e.g. from FMP's constituent endpoint
 * once/if that entitlement is confirmed, or any other reliable source).
 */
export const SP500_TICKERS: string[] = [
  ...
  "WELL", "WST", "WDC", "WY", "WSM", "WMB", "WTW", "WYNN", "XEL", "XYL",
  "YUM", "ZBRA", "ZBH", "ZTS",
];
