...
const log = logger.child("api:sentiment");
...
export async function GET(_req: NextRequest, { params }: { params: { ticker: string } }) {
  const result = await runSentimentAnalysis(params.ticker);

  if (!result.ok) {
    const status = STATUS_BY_ERROR_CODE[result.error.code] ?? 502;
    log.warn("sentiment analysis request failed", { ticker: params.ticker, error: result.error });
