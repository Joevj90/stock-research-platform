...
const log = logger.child("api:valuation");
...
export async function GET(_req: NextRequest, { params }: { params: { ticker: string } }) {
  const result = await runValuationAnalysis(params.ticker);

  if (!result.ok) {
    const status = STATUS_BY_ERROR_CODE[result.error.code] ?? 502;
    log.warn("valuation request failed", { ticker: params.ticker, error: result.error });
